...
public class Grid extends View implements Serializable {
    ...
    public void resetCells(ArrayList<Integer> colors) {
        if (grid_lines == -1 || grid_columns == -1)
            throw new IllegalStateException("You should call init() before trying to initialise the cells");
    ...
    public void setCells(ArrayList<Cell> cells) {
        ...
        if (!isValid)
            throw new IllegalArgumentException("The cells variable contains an error : " + errorMessage);
